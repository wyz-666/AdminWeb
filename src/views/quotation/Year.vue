...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "CEA当月年度报价");
      XLSX.utils.book_append_sheet(workbook, worksheet1, "CEA当月年度报价均值");
      // 3. 写出文件数据
      const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
      // 4. 保存文件
      const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
      saveAs(blob, `CEA当月年度报价及均值-${dayjs(this.date).format("YYYY-MM-DD")}.xlsx`);
    },
  },
};
</script>
