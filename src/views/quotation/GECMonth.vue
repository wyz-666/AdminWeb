<template>
  <el-row :gutter="12">
    ...
    <el-button type="success" @click="exportExcel" style="margin-left: 10px;">导出GEC当月报价及均值</el-button>
    <el-col :span="24">
      ...
      <el-card shadow="always" style="border-top: none;">
        <div style="font-size: 16px; font-weight: bold; margin-bottom: 10px;">各类型平均报价</div>
      ...
      <el-dialog title="提交GEC预测结果" v-model="dialogFormVisible" width="40%">
        <el-form :model="expectation">
          <el-date-picker v-model="expectation.date" type="month" placeholder="选择月份" />
          <el-select v-model="expectation.type" placeholder="请选择产品类型" size="large" style="width: 240px;margin-top: 20px;">
            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
          <el-form-item label="价格" :label-width="formLabelWidth" style="margin-top: 20px;">
        ...
        <template #footer>
          <span class="dialog-footer">
            ...
            <el-button size="mini" type="primary" @click="uploadExpectation">提交预测值</el-button>
          </span>
        </template>
      </el-dialog>
    </el-col>
    ...
    getAVG() {
      ...
      this.$axios.get('/admin/getMonthQuotation', {
        ...
        for (let type in typeMap) {
          let avg = typeMap[type].sum / typeMap[type].count;
          avgResult.push({
            type: type,
